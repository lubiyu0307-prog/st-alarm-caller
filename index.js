...
function buildAlarmRule() {
    return [
        ...
        "若使用者明確要求，請先用你自己的語氣正常回覆、答應對方，並在劇情中描寫你「設定鬧鐘」的動作；描寫完那個動作的段落後，立刻「另起一行」單獨輸出一行如下格式的標記，然後才繼續寫剩下的正文：",
        "[[ALARM|HH:MM|鬧鐘名稱]]",
        "規則：",
        "- 時間一律使用24小時制（HH:MM）",
        "- 鬧鐘名稱由你依照自己的語氣與你平常對使用者的稱呼現場撰寫，需貼合用途（例如叫醒、提醒事項、休息、喝水吃飯等），限10字以內，不使用emoji，讀起來像一張你留在鬧鐘裡的小紙條",
        "- 標記必須單獨佔一行，緊跟在你描寫設鬧鐘動作的那個段落正下方；若劇情中沒有具體描寫設鬧鐘的動作，就放在正文結尾、狀態欄或其他固定格式區塊之前。它是系統標記，不受「最後一行必須是某格式」之類規則影響",
